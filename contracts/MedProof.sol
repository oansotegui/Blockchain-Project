...
contract MedProof {
    ...
    enum ActorType { Manufacturer, Distributor, Pharmacy }

    // Estructura para representar un actor/participante
    struct Actor {
        string name;         // Nombre del actor
        uint actorType;      // 0: Productor, 1: Distribuidor, 2: Farmacia
    ...
    event ActorRegistered(address indexed actorAddress, string name, uint actorType);
    // Evento para registrar un nuevo medicamento 
    event MedicationRegistered(string medicationID, address registeredBy, bytes32 detailsHash);
    // Evento para notificar una transferencia de medicamento entre actores
    event MedicationTransferred(string indexed medicationID, address indexed fromAddress, address indexed toAddress);
    ...
    function registerActor(address _actorAddress, string memory _name, uint _actorType) public onlyOwner {
        ...
        require(uint(_actorType) <= 2, "Tipo de actor invalido");

        // Crear y almacenar el actor
        actors[_actorAddress] = Actor(_name, uint(_actorType), true);

        // Emitir un evento para notificar el registro exitoso
        emit ActorRegistered(_actorAddress, _name, uint(_actorType));
    }

    // Función para registrar un medicamento
    function registerMedication(string memory _medicationID, bytes32 _detailsHash) public onlyRegisteredActors {
        require(bytes(medications[_medicationID].medicationID).length == 0, "Medication already registered");
    ...
    function transferMedication(string memory _medicationID, address _toAddress) public onlyRegisteredActors {
        // Asegurarse de que el medicamento exista
        require(bytes(medications[_medicationID].medicationID).length != 0, "Medication does not exist");
        
        // Asegurarse de que el actor que intenta transferir el medicamento es quien lo tiene en posesión
        require(medications[_medicationID].currentHolder == msg.sender, "You do not have possession of this medication");
        
        // Asegurarse de que el actor al que se transfiere está registrado
        require(actors[_toAddress].isRegistered, "Recipient actor is not registered");
    ...
    function sellMedication(string memory _medicationID) public onlyRegisteredActors {
        // Asegurarse de que el medicamento exista
        require(bytes(medications[_medicationID].medicationID).length != 0, "Medication does not exist");
        
        // Asegurarse de que el actor que intenta vender el medicamento es una farmacia y lo tiene en posesión
        require(medications[_medicationID].currentHolder == msg.sender && actors[msg.sender].actorType == uint(ActorType.Pharmacy), "Either you do not have possession of this medication or you are not a pharmacy");
    ...
    function verifyMedication(string memory _medicationID) public view returns (address registeredBy, bytes32 detailsHash, bool isValidated) {
        Medication memory medication = medications[_medicationID];

        require(bytes(medication.medicationID).length != 0, "Medicamento no registrado");

        return (medication.registeredBy, medication.detailsHash, medication.isValidated);
    }
    ...
    function validateMedication(string memory _medicationID) public onlyOwner {
        require(bytes(medications[_medicationID].medicationID).length != 0, "Medication does not exist");
        require(!medications[_medicationID].isValidated, "Medication already validated");
